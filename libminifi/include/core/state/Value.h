...
/*
template<typename T,  typename std::enable_if<std::is_same<std::string, T>::value, std::shared_ptr<Value>>::type = 0>
static std::shared_ptr<Value> createValue(
    const T &object) {
  return std::make_shared<Value>(object);
}
...
template<typename T,  typename std::enable_if<std::is_same<int, T>::value, std::shared_ptr<Value>>::type = 0>
static std::shared_ptr<Value> createValue(
    const T &object) {
  return std::make_shared<IntValue>(object);
}

template<typename T,  typename std::enable_if<std::is_same<bool, T>::value, std::shared_ptr<Value>>::type = 0>
static std::shared_ptr<Value> createValue(
    const T &object) {
  return std::make_shared<BoolValue>(object);
}

template<typename T,  typename std::enable_if<std::is_same<uint64_t, T>::value, std::shared_ptr<Value>>::type = 0>
static std::shared_ptr<Value> createValue(
    const T &object) {
  return std::make_shared<Int64Value>(object);
}
