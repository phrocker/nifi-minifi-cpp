...
class MetricsListener {
 public:
  MetricsListener(const std::shared_ptr<metrics::MetricsSource> &source, const std::shared_ptr<metrics::MetricsSink> &sink)
      : running_(true),
        sink_(sink),
        source_(source) {
    ...
    function_ = [&]() {
      while(running_) {
        ...
        if (nullptr != source_ && nullptr != sink_) {
          source_->getMetrics(metric_vector);
 ...
 private:
  ...
  std::atomic<bool> running_;
  std::shared_ptr<metrics::MetricsSource> source_;
