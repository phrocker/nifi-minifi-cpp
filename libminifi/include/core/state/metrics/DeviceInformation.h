...
class Device {
 public:
  ...
  void initialize() {
    ...
    std::string loopback = "localhost";
    char hostname[1024];
    hostname[1023] = '\0';
    gethostname(hostname, 1023);
    int errcode = getaddrinfo(hostname, 0, &hints, &addr_info_);
    ...
    struct hostent *h;
#ifdef __MACH__
    h = gethostbyname(loopback.c_str());
#else
    const char *host;
    ...
    for (; p != NULL; p = p->ai_next) {
      if (IsNullOrEmpty(canonical_hostname_)) {
        if (!IsNullOrEmpty(p) && !IsNullOrEmpty(p->ai_canonname)) {
          if (!IsNullOrEmpty(p->ai_addr)) {

            char ipstr[INET_ADDRSTRLEN];
            inet_ntop(AF_INET, &((struct sockaddr_in *) p->ai_addr)->sin_addr, ipstr, INET_ADDRSTRLEN);
            ip_ = ipstr;
