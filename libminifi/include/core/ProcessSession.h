...
class ProcessSession {
 public:
  ...
  ProcessSession(ProcessContext *processContext = NULL)
    ...
    provenance_report_ = new ProvenanceReporter(
  ...
  std::shared_ptr<core::Record>  get();
  //! Create a new UUID FlowFile with no content resource claim and without parent
  std::shared_ptr<core::Record> create();
  //! Create a new UUID FlowFile with no content resource claim and inherit all attributes from parent
  std::shared_ptr<core::Record> create(std::shared_ptr<core::Record> &parent);
  //! Clone a new UUID FlowFile from parent both for content resource claim and attributes
  std::shared_ptr<core::Record> clone(std::shared_ptr<core::Record> &parent);
  //! Clone a new UUID FlowFile from parent for attributes and sub set of parent content resource claim
  std::shared_ptr<core::Record> clone(std::shared_ptr<core::Record> &parent, long offset, long size);
  //! Duplicate a FlowFile with the same UUID and all attributes and content resource claim for the roll back of the session
  std::shared_ptr<core::Record> duplicate(std::shared_ptr<core::Record> &orignal);
  //! Transfer the FlowFile to the relationship
  void transfer(std::shared_ptr<core::Record> &flow, Relationship relationship);
  void transfer(std::shared_ptr<core::Record> &&flow, Relationship relationship);
  //! Put Attribute
  void putAttribute(std::shared_ptr<core::Record> &flow, std::string key, std::string value);
  void putAttribute(std::shared_ptr<core::Record> &&flow, std::string key, std::string value);
  //! Remove Attribute
  void removeAttribute(std::shared_ptr<core::Record> &flow, std::string key);
  void removeAttribute(std::shared_ptr<core::Record> &&flow, std::string key);
  //! Remove Flow File
  void remove(std::shared_ptr<core::Record> &flow);
  void remove(std::shared_ptr<core::Record> &&flow);
  //! Execute the given read callback against the content
  void read(std::shared_ptr<core::Record> &flow, InputStreamCallback *callback);
  void read(std::shared_ptr<core::Record> &&flow, InputStreamCallback *callback);
  //! Execute the given write callback against the content
  void write(std::shared_ptr<core::Record> &flow, OutputStreamCallback *callback);
  void write(std::shared_ptr<core::Record> &&flow, OutputStreamCallback *callback);
  //! Execute the given write/append callback against the content
  void append(std::shared_ptr<core::Record> &flow, OutputStreamCallback *callback);
  void append(std::shared_ptr<core::Record> &&flow, OutputStreamCallback *callback);
  //! Penalize the flow
  void penalize(std::shared_ptr<core::Record> &flow);
  void penalize(std::shared_ptr<core::Record> &&flow);
  //! Import the existed file into the flow
  void import(std::string source, std::shared_ptr<core::Record> &flow, bool keepSource = true,
              uint64_t offset = 0);
  void import(std::string source, std::shared_ptr<core::Record> &&flow, bool keepSource = true,
              uint64_t offset = 0);
 ...
 protected:
  ...
  std::map<std::string, std::shared_ptr<core::Record> > _updatedFlowFiles;
  //! Copy of the original FlowFiles being modified by current process session as above
  std::map<std::string, std::shared_ptr<core::Record> > _originalFlowFiles;
  //! FlowFiles being added by current process session
  std::map<std::string, std::shared_ptr<core::Record> > _addedFlowFiles;
  //! FlowFiles being deleted by current process session
  std::map<std::string, std::shared_ptr<core::Record> > _deletedFlowFiles;
  ...
  std::map<std::string, std::shared_ptr<core::Record> > _clonedFlowFiles;

 private:
  // Clone the flow file during transfer to multiple connections for a relationship
  std::shared_ptr<core::Record> cloneDuringTransfer(std::shared_ptr<core::Record> &parent);
  //! ProcessContext
  ProcessContext *process_context_;
  //! Logger
  std::shared_ptr<logging::Logger> logger_;
  //! Provenance Report
  ProvenanceReporter *provenance_report_;
