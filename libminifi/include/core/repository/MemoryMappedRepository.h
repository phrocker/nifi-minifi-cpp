...
/**
 * MemoryMappedRepository is a content repository that stores data onto the local file system.
 */
class MemoryMappedRepository : public core::ContentRepository, public core::CoreComponent {
 public:
  MemoryMappedRepository(std::string name = getClassName<MemoryMappedRepository>())
      : core::CoreComponent(name),
        logger_(logging::LoggerFactory<MemoryMappedRepository>::getLogger()) {

  }
  virtual ~MemoryMappedRepository() {
...
#endif /* LIBMINIFI_INCLUDE_CORE_REPOSITORY_MMAPCONTENTREPOSITORY_H */
