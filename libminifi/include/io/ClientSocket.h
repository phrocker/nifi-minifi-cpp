...
class Socket : public BaseStream {
 public:
   ...
   */
  virtual int readData(std::vector<uint8_t> &buf, int buflen, bool retrieve_all_bytes = true);
  /**
   * Reads data and places it into buf
   * @param buf buffer in which we extract data
   * @param buflen
   * @param retrieve_all_bytes determines if we should read all bytes before returning
   */
  virtual int readData(uint8_t *buf, int buflen, bool retrieve_all_bytes = true);
