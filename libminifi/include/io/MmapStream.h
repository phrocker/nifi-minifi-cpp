 ...
 */
#ifndef LIBMINIFI_INCLUDE_IO_MMAPSTREAM_H_
#define LIBMINIFI_INCLUDE_IO_MMAPSTREAM_H_
...
class MmapStream : public io::FileStream {
 public:
   ...
   */
  explicit MmapStream(const std::string &path, uint32_t offset, bool write_enable = false);

  /**
   * File Stream constructor that accepts an fstream shared pointer.
   * It must already be initialized for read and write.
   */
  explicit MmapStream(const std::string &path);
...
#endif /* LIBMINIFI_INCLUDE_IO_MMAPSTREAM_H_ */
