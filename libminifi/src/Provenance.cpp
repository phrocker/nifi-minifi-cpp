...
void ProvenanceReporter::create(
...
void ProvenanceReporter::modifyAttributes(
    std::shared_ptr<core::Record> flow,
    std::string detail) {
...
void ProvenanceReporter::modifyContent(
    std::shared_ptr<core::Record> flow,
...
void ProvenanceReporter::clone(
    std::shared_ptr<core::Record> parent,
...
void ProvenanceReporter::join(
    std::vector<std::shared_ptr<core::Record> > parents,
    std::shared_ptr<core::Record> child,
    std::string detail, uint64_t processingDuration) {
  ...
  if (event) {
    event->addChildFlowFile(child);
    std::vector<std::shared_ptr<core::Record> >::iterator it;
    for (it = parents.begin(); it != parents.end(); it++) {
      std::shared_ptr<core::Record> record = *it;
...
void ProvenanceReporter::fork(
    std::vector<std::shared_ptr<core::Record> > child,
    std::shared_ptr<core::Record> parent,
    std::string detail, uint64_t processingDuration) {
...
void ProvenanceReporter::expire(
    std::shared_ptr<core::Record> flow,
    std::string detail) {
...
void ProvenanceReporter::send(
    std::shared_ptr<core::Record> flow,
    std::string transitUri, std::string detail, uint64_t processingDuration,
    bool force) {
...
void ProvenanceReporter::receive(
    std::shared_ptr<core::Record> flow,
    std::string transitUri, std::string sourceSystemFlowFileIdentifier,
    std::string detail, uint64_t processingDuration) {
...
uint64_t ProvenanceRepository::repo_size_ = 0;
...
void ProvenanceRepository::run(ProvenanceRepository *repo) {
  // threshold for purge
  uint64_t purgeThreshold = repo->max_partition_bytes_ * 3 / 4;
  while (repo->running_) {
    std::this_thread::sleep_for(std::chrono::milliseconds(repo->purge_period_));
    uint64_t curTime = getTimeMillis();
    uint64_t size = repo->repoSize();
    if (size >= purgeThreshold) {
      std::vector<std::string> purgeList;
      leveldb::Iterator* it = repo->db_->NewIterator(leveldb::ReadOptions());
      for (it->SeekToFirst(); it->Valid(); it->Next()) {
        ProvenanceEventRecord eventRead;
        std::string key = it->key().ToString();
        if (eventRead.DeSerialize((uint8_t *) it->value().data(),
                                  (int) it->value().size())) {
          if ((curTime - eventRead.getEventTime()) > repo->max_partition_millis_)
    ...
    if (size > repo->max_partition_bytes_)
      repo->repo_full_ = true;
    else
      repo->repo_full_ = false;
  }
  return;
}
