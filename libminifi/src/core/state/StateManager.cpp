 ...
 * Metrics operations
 */
int16_t StateManager::getMetrics(std::vector<std::shared_ptr<metrics::Metrics>> &metric_vector) {
  auto now = std::chrono::steady_clock::now();
...
bool StateManager::registerUpdateListener(const std::shared_ptr<UpdateController> &updateController) {
  ...
  for (auto function : functions) {
    std::unique_ptr<utils::AfterExecute<Update>> after_execute = std::unique_ptr<utils::AfterExecute<Update>>(new UpdateRunner(isRunning()));
    utils::Worker<Update> functor(function, std::move(after_execute));
...
bool StateManager::startMetrics() {
  std::unique_ptr<utils::AfterExecute<Update>> after_execute = std::unique_ptr<utils::AfterExecute<Update>>(new UpdateRunner(isRunning()));
  utils::Worker<Update> functor(metrics_listener_->getFunction(), std::move(after_execute));
