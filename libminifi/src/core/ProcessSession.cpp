...
std::shared_ptr<core::Record> ProcessSession::create() {
...
std::shared_ptr<core::Record> ProcessSession::clone(
    std::shared_ptr<core::Record> &parent) {
  std::shared_ptr<core::Record> record =
      this->create(parent);
  if (record) {
...
std::shared_ptr<core::Record> ProcessSession::cloneDuringTransfer(
    std::shared_ptr<core::Record> &parent) {
  std::map<std::string, std::string> empty;
  std::shared_ptr<core::Record> record =
      std::make_shared<FlowFileRecord>(empty);
  ...
  if (record) {
    ...
    if (parent->getResourceClaim()) {
      if ((offset + size) > (long) parent->getSize()) {
        ...
        std::map<std::string,
            std::shared_ptr<core::Record> >::iterator it =
        ...
        if (connections.empty()) {
          ...
          for (std::set<std::shared_ptr<Connectable>>::iterator itConnection =
            ...
            if (itConnection == connections.begin()) {
              ...
              std::shared_ptr<core::Record> cloneRecord;
...
std::shared_ptr<core::Record> ProcessSession::get() {
  ...
  do {
    std::set<std::shared_ptr<core::Record> > expired;
    std::shared_ptr<core::Record> ret =
        current->poll(expired);
    if (expired.size() > 0) {
      // Remove expired flow record
      for (std::set<std::shared_ptr<core::Record> >::iterator it =
    ...
    if (ret) {
      ...
      std::map<std::string, std::string> empty;
      std::shared_ptr<core::Record> snapshot =
          std::make_shared<FlowFileRecord>(empty);
