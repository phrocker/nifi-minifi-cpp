 ...
 */
void Record::setDeleted(const bool deleted) {
  marked_delete_ = deleted;
}

std::shared_ptr<ResourceClaim> Record::getResourceClaim() {
  return claim_;
}

void Record::clearResourceClaim() {
  claim_ = nullptr;
}
void Record::setResourceClaim(std::shared_ptr<ResourceClaim> &claim) {
  claim_ = claim;
}

// ! Get Entry Date
uint64_t Record::getEntryDate() {
  return entry_date_;
}
// ! Get Lineage Start Date
uint64_t Record::getlineageStartDate() {
  return lineage_start_date_;
}

std::set<std::string> &Record::getlineageIdentifiers() {
  return lineage_Identifiers_;
}

bool Record::getAttribute(std::string key, std::string &value) {
...
// ! Get Offset
uint64_t Record::getOffset() {
  return offset_;
}

bool Record::removeAttribute(const std::string key) {
  auto it = attributes_.find(key);
  if (it != attributes_.end()) {
...
bool Record::updateAttribute(const std::string key, const std::string value) {
  auto it = attributes_.find(key);
  if (it != attributes_.end()) {
...
bool Record::addAttribute(const std::string &key, const std::string &value) {
  auto it = attributes_.find(key);
  if (it != attributes_.end()) {
...
void Record::setLineageStartDate(const uint64_t date) {
 ...
 */
void Record::setOriginalConnection(
 ...
 */
std::shared_ptr<core::Connectable> Record::getConnection() {
  return connection_;
}
 ...
 */
std::shared_ptr<core::Connectable> Record::getOriginalConnection() {
