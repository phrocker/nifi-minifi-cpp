...
core::Property GetTCP::EndpointList(
                                    "endpoint-list",
                                    "A comma delimited list of the endpoints to connect to. The format should be <server_address>:<port>.",
                                    "");
core::Property GetTCP::ConcurrentHandlers(
                                          "concurrent-handler-count",
                                          "Number of concurrent handlers for this session", "2");
...
int16_t DataHandler::handle(std::string source, uint8_t *message, size_t size, bool partial) {
  std::unique_ptr<core::ProcessSession> my_session = sessionFactory_->createSession();
...
void GetTCP::onSchedule(core::ProcessContext *context, core::ProcessSessionFactory *sessionFactory) {
  ...
  if (context->getProperty(ConnectionAttemptLimit.getName(), value)) {
    ...
    connectionAttemptLimit = connects;
  }
  ...
  f_ex = [&] {
    ...
    // reuse the byte buffer.
      ...
      do {
        logger_->log_info("Doing stuff");
        if ( socket_ring_buffer_.try_dequeue(socket_ptr) ) {
          logger_->log_info("Doing from dequeued socket");
          int size_read = socket_ptr->readData(buffer, 16*1024*1024, false);
          logger_->log_info("Reading %i bytes", size_read);
          if (size_read >= 0) {
            if (size_read > 0) {
  ...
  bool useNonBlocking = endpoints.size() > concurrent_handlers_;
  for (auto endpoint : endpoints) {
    std::vector<std::string> hostAndPort = utils::StringUtils::split(endpoint, ":");
    if (hostAndPort.size() == 2) {
      logger_->log_debug("Opening socket to %s:%s", hostAndPort.at(0), hostAndPort.at(1));
...
void GetTCP::onTrigger(core::ProcessContext *context, core::ProcessSession *session) {
  ...
  for (auto &endpoint : endpoints) {
    ...
    if (endPointFuture == live_clients_.end()) {
      ...
      std::unique_ptr<utils::AfterExecute<int>> after_execute = std::unique_ptr<utils::AfterExecute<int>>(new SocketAfterExecute(running_));
