...
int HttpConfigurationListener::pemPassWordCb(char *buf, int size, int rwflag, void *param) {
  minifi::HttpConfigurationListener *listener = static_cast<minifi::HttpConfigurationListener*>(param);

  if (listener->passphrase_.length() > 0) {
    memset(buf, 0x00, size);
    memcpy(buf, listener->passphrase_.c_str(), listener->passphrase_.length() - 1);
    return listener->passphrase_.length() - 1;
  }
  return 0;
}

CURLcode HttpConfigurationListener::configureSSLContext(CURL *curl, void *ctx, void *param) {
  minifi::HttpConfigurationListener *listener = static_cast<minifi::HttpConfigurationListener*>(param);
  ...
  SSL_FILETYPE_PEM);
  SSL_CTX_set_default_passwd_cb(sslCtx, HttpConfigurationListener::pemPassWordCb);
  SSL_CTX_set_default_passwd_cb_userdata(sslCtx, param);
  SSL_CTX_use_PrivateKey_file(sslCtx, listener->private_key_.c_str(),
  SSL_FILETYPE_PEM);
  // verify private key
  if (!SSL_CTX_check_private_key(sslCtx)) {
    listener->logger_->log_error("Private key does not match the public certificate, error : %s", std::strerror(errno));
    return CURLE_FAILED_INIT;
  }

  listener->logger_->log_debug("HttpConfigurationListener load Client Certificates OK");
  return CURLE_OK;
}
...
void HttpConfigurationListener::configureSecureConnection(CURL *http_session) {
  ...
  if (this->need_client_certificate_) {
    CURLcode ret;
    ret = curl_easy_setopt(http_session, CURLOPT_SSL_CTX_FUNCTION, &HttpConfigurationListener::configureSSLContext);
    if (ret != CURLE_OK)
      logger_->log_error("CURLOPT_SSL_CTX_FUNCTION not supported %d", ret);
    curl_easy_setopt(http_session, CURLOPT_SSL_CTX_DATA, static_cast<void*>(this));
