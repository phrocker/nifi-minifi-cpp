...
void FlowFileRecord::releaseClaim(std::shared_ptr<ResourceClaim> claim) {
  ...
  std::string value;
  logger_->log_debug("Delete Resource Claim %s, %s, attempt %ll", getUUIDStr(), claim_->getContentFullPath().c_str(), claim_->getFlowFileRecordOwnedCount());
...
bool FlowFileRecord::DeSerialize(std::string key) {
  ...
  if (!ret) {
    ...
    logger_->log_debug("NiFi FlowFile Read event %s length %ll", key.c_str(), value.length());
  }
  ...
  if (ret) {
    logger_->log_debug("NiFi FlowFile retrieve uuid %s size %ll connection %s success", uuidStr_.c_str(), stream.getSize(), uuid_connection_.c_str());
  } else {
    logger_->log_debug("NiFi FlowFile retrieve uuid %s size %ll connection %ll fail", uuidStr_.c_str(), stream.getSize(), uuid_connection_.c_str());
  }
...
bool FlowFileRecord::Serialize() {
  ...
  if (flow_repository_->Put(uuidStr_, const_cast<uint8_t*>(outStream.getBuffer()), outStream.getSize())) {
    logger_->log_debug("NiFi FlowFile Store event %s size %ll success", uuidStr_.c_str(), outStream.getSize());
    return true;
  } else {
    logger_->log_error("NiFi FlowFile Store event %s size %ll fail", uuidStr_.c_str(), outStream.getSize());
    return false;
  }
