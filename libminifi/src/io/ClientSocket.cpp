...
int Socket::readLongLong(uint64_t &value, bool is_little_endian) {
...
int Socket::readLong(uint32_t &value, bool is_little_endian) {
...
int Socket::readShort(uint16_t &value, bool is_little_endian) {
...
int Socket::readData(uint8_t *buf, int buflen) {
	...
	int total_read = 0;
	logger_->log_info("reading %i",buflen);
	while (buflen) {
		...
		logger_->log_info("Bytes read is %i",bytes_read);
		if (bytes_read <= 0) {
			if (bytes_read == 0)
