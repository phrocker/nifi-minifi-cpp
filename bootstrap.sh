...
save_state(){
   ...
   echo "BUILD_IDENTIFIER=\"${BUILD_IDENTIFIER}\"" >> ${script_directory}/state
   echo "BUILD_DIR=\"${BUILD_DIR}\"" >> ${script_directory}/state 
   for option in "${OPTIONS[@]}" ; do
      option_value="${!option}"
      echo "$option=\"$option_value\"" >> ${script_directory}/state
   done
}
