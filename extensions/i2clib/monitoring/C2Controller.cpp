...
C2Controller::C2Controller(std::string name, uuid_t uuid)
    : HeartBeatReporter(name, uuid),
      logger_(logging::LoggerFactory<C2Controller>::getLogger()) {
}

void C2Controller::initialize(const std::shared_ptr<core::controller::ControllerServiceProvider> &controller, const std::shared_ptr<Configure> &configure) {
  HeartBeatReporter::initialize(controller, configure);
  logger_->log_debug("Initializing c2 pi car controller");
  if (nullptr != configuration_) {
    ...
    if (!listeningPort.empty() && !rootUri.empty()) {
      handler = std::unique_ptr<PiCarProtocol>(new PiCarProtocol());
      if (!caCert.empty()) {
...
int16_t C2Controller::heartbeat(const C2Payload &payload) {
...
std::unique_ptr<CivetServer> C2Controller::start_webserver(const std::string &port, std::string &rooturi, CivetHandler *handler, std::string &ca_cert) {
  struct mg_callbacks callback;

  memset(&callback, 0, sizeof(callback));
  callback.init_ssl = c2_ssl_protocol_en;
  std::string my_port = port;
  my_port += "s";
  callback.log_message = log_c2_message;
...
std::unique_ptr<CivetServer> C2Controller::start_webserver(const std::string &port, std::string &rooturi, CivetHandler *handler) {
