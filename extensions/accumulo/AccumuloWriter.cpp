...
void AccumuloWriter::onTrigger(core::ProcessContext *context, core::ProcessSession *session) {
  ...
  do {
    ...
    if (!flow || output_dir_.empty()) {
      logger_->log_info("Exit Accumulo Writer: no flow or %s is empty",output_dir_);
      return;
    }
    ...
    if (current_count_ > max_keys_) {
      ...
      auto rfile_flow = session->create();

      rfile_flow->addAttribute("Rfile", outputDir);


      session->import(rfile,rfile_flow,true,0);

      session->transfer(rfile_flow, Import);

      session->transfer(flow, Success);

      logger_->log_info("rfile flow file size %d, flowfile size %d", rfile_flow->getSize(), flow->getSize());
      return;
    }
